...
export default async function ThSocialMediaPage() {
  const allClients = getAllClients()
  const proofClients = allClients.filter((c) => c.cover).slice(0, 6)
  const t = th.socialMedia

  const platforms = t.platforms.map((p, i) => ({ ...p, ...platformIcons[i] }))
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
              {t.deliverables.map((d) => (
                <div key={d.label} className="text-center">
                  <div className="font-sora font-extrabold text-3xl md:text-4xl text-green mb-1">{d.value}</div>
                  <div className="font-poppins text-xs text-white/50 leading-snug">{d.label}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <section className="max-w-site mx-auto px-5 md:px-12 pb-20">
            ...
            <div className="flex items-end justify-between gap-4 mb-8">
              <h2 className="font-sora font-extrabold text-3xl md:text-4xl text-dark tracking-tight">
                {t.ourWorkH2a}<br className="hidden md:block" /> {t.ourWorkH2b}
              </h2>
              <Link href="/th/work" className="shrink-0 font-poppins text-sm font-semibold text-green hover:underline">
                {t.viewAll}
              </Link>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 max-w-3xl">
            {t.process.map((s) => (
              <div key={s.num} className="bg-white rounded-card p-7">
                <span className="font-sora font-extrabold text-4xl text-green/20 block mb-4">{s.num}</span>
