...
export default async function ThGrabSalesPage() {
  const allClients = getAllClients()
  const proofClients = allClients.filter((c) => c.cover).slice(0, 6)
  const t = th.grabSales
      ...
      <main className="min-h-[100dvh] bg-[#EDEDED]">
        ...
        <section className="max-w-site mx-auto px-5 md:px-12 pt-16 pb-20 md:pt-24 md:pb-28">
          <div className="max-w-2xl">
            ...
            <h1 className="font-sora font-extrabold text-[clamp(2.4rem,5vw,3.6rem)] leading-[1.1] tracking-tight text-dark mb-6">
              {t.h1Line1}<br />
              <span className="text-green">{t.h1Line2}</span>
            </h1>
            <p className="font-poppins text-base md:text-lg text-muted leading-relaxed mb-8 max-w-xl">
              {t.subheading}
            </p>
            <div className="flex flex-wrap gap-3">
              <Link href="/th/contact" className="bg-green text-white font-poppins font-semibold text-sm px-6 py-3.5 rounded-pill hover:brightness-105 transition-all active:scale-[0.98]">
                {t.applyNow}
              </Link>
            </div>
          </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
              {t.stats.map((s) => (
                <div key={s.label} className="text-center">
                  <div className="font-sora font-extrabold text-3xl md:text-4xl text-green mb-1">{s.value}</div>
                  <div className="font-poppins text-xs text-white/50 leading-snug">{s.label}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="py-20 md:py-24">
          <div className="max-w-site mx-auto px-5 md:px-12">
            ...
            <p className="font-poppins text-sm text-muted mb-12">{t.resultsSubtext}</p>
            ...
            >
              {[
                { before: '฿665K', after: '฿1.25M', timeframe: '2', growth: '1.9x growth', monthly: '+฿295K / month', beforeImg: '/results/proof-1-after.jpg', afterImg: '/results/proof-1-before.jpg' },
                { before: '฿300K', after: '฿628K',  timeframe: '2', growth: '2.1x growth', monthly: '+฿328K / month', beforeImg: '/results/proof-2-before.jpg', afterImg: '/results/proof-2-after.jpg' },
                { before: '฿127K', after: '฿249K',  timeframe: '4', growth: '2x growth',   monthly: '+฿122K / month', beforeImg: '/results/proof-3-before.jpg', afterImg: '/results/proof-3-after.jpg' },
                { before: '฿431K', after: '฿814K',  timeframe: '5', growth: '1.9x growth', monthly: '+฿383K / month', beforeImg: '/results/proof-4-before.jpg', afterImg: '/results/proof-4-after.jpg' },
              ].map((card) => (
                <div key={card.before} className="shrink-0 w-[80vw] md:w-auto bg-white rounded-[20px] shadow-[0_2px_16px_rgba(0,0,0,0.06)] p-5" style={{ scrollSnapAlign: 'start' }}>
                  ...
                  <p className="font-poppins text-[11px] text-muted/70 mb-4">
                    {t.resultsIn} {card.timeframe} เดือน
                  </p>
                  ...
                  <p className="font-poppins text-[11px] text-muted/70">{t.avgMonthly}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {t.steps.map((s) => (
              <div key={s.num} className="bg-white rounded-card p-7">
                <span className="font-sora font-extrabold text-4xl text-green/50 block mb-4">{s.num}</span>
        ...
        {proofClients.length > 0 && (
          <section className="max-w-site mx-auto px-5 md:px-12 pb-20">
            ...
            <div className="mt-4 mb-8">
              <Link href="/th/work" className="font-poppins text-sm font-semibold text-green hover:underline">
                {t.viewAll}
              </Link>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {proofClients.map((c) => (
                <Link key={c.slug} href={`/th/work/${c.slug}`} className="group relative rounded-[16px] overflow-hidden bg-[#2D2D2D] aspect-[3/4]">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={c.cover!} alt={c.name} className="w-full h-full object-cover group-hover:scale-[1.03] transition-transform duration-500" />
            ...
            <div className="mt-8 flex justify-center">
              <Link href="/th/work" className="inline-block bg-white text-dark font-poppins font-semibold text-sm px-6 py-3.5 rounded-pill border border-black/10 hover:border-black/20 transition-all active:scale-[0.98]">
                {t.seeMore}
              </Link>
            </div>
          </section>
        ...
        <PricingCard lang="th" contextNote="บริการ Grab เฉพาะลูกค้าที่ทำโซเชียลมีเดียด้วยเท่านั้น" />
        ...
        <section className="max-w-site mx-auto px-5 md:px-12 pb-24">
          <div className="bg-green rounded-card p-8 md:p-12 text-center">
            ...
            <Link href="/th/contact" className="inline-block bg-white text-dark font-poppins font-semibold text-sm px-8 py-4 rounded-pill hover:bg-white/90 transition-all active:scale-[0.98]">
              {t.ctaButton}
            </Link>
          </div>
        </section>
