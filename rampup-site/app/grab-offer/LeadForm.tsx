...
export default function LeadForm() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!webhookSent.current) {
        ...
        if (process.env.NODE_ENV !== 'production') {
          console.info('[Lead] fired', { event_id, page_type: 'funnel-grab-offer', fbq: typeof (window as any).fbq === 'function' })
        }
        fetch('/api/lead-relay', {
          ...
          body: JSON.stringify({
            ...
            page_type:    'funnel-grab-offer',
            form_name:    'grab_offer_funnel',
  ...
  if (submitted) {
    return (
      <div className="text-center py-10">
        ...
        <h3 className="font-sora font-bold text-xl text-dark mb-2">Application received</h3>
        <p className="font-poppins text-base text-muted">We&apos;ll review your Grab store and reach out within 24 hours.</p>
      </div>
    )
  }
  ...
  return (
    ...
    >
      <input type="hidden" name="form-name" value="grab_offer_funnel" />
      <input type="hidden" name="source" value="grab_offer" />
      ...
      <div>
        <label className={labelClass}>Current Grab monthly sales (rough)</label>
      ...
      <div>
        ...
        <input name="phone" type="text" required placeholder="08X-XXX-XXXX or LINE ID" className={inputClass} />
      </div>
      ...
      <button type="submit" disabled={loading}
        ...
        {loading ? 'Sending…' : 'Apply for my free Grab audit'}
      </button>
      <p className="font-poppins text-sm text-muted italic text-center">
        Free audit. No obligation. We&apos;ll tell you honestly if it&apos;s worth it.
      </p>
    </form>
  )
}
