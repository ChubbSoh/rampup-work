...
resource: a mistake in one project is visible to every staff member at once.

**Sending is one HTTP call to an internal relay.** The endpoint, the credential
and the full contract are in the private platform repo — this repo is public, so
they are deliberately not written here:

`ChubbSoh/rampup-line-gateway` → `LINE-INTEGRATION.md`

**Never do any of these:**

| Never | Why |
|---|---|
| Mount the LINE channel access token or channel secret | Two services holding the token is the defect the platform exists to remove |
| `npm i @line/bot-sdk` to send a message | Reaching for the SDK is what leads to holding the token |
| Create or link a rich menu | `rampup-leave` owns menus; a default menu changes what **every** staff member sees |
| Add a `/webhook` route | The channel has one webhook URL and it already points somewhere. Any other is dead code |
| Call a platform service directly | They are IAM-gated; use the relay |

**Three behaviours that will bite you**, none of them obvious:

1. **At most 5 messages per call.** More are silently dropped and you still get a
   success response.
2. **No retry is built in.** Rate limiting surfaces as a 500; back off
...
Read the platform doc rather than copying what another service does — every rule
